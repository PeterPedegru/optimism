...
type simpleVirtualNode struct {
	...
	vnID         string
	initOverload *rollupNode.InitOverload // Shared resources
}
...
func NewVirtualNode(cfg *opnodecfg.Config, log gethlog.Logger, initOverload *rollupNode.InitOverload) *simpleVirtualNode {
...
func (v *simpleVirtualNode) Start(ctx context.Context) error {
	...
	opNode, err := e2eopnode.NewOpnodeWithOverload(v.log, v.cfg, func(err error) {
		if err != nil {
